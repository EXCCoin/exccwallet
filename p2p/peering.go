...
// DialFunc dials a network connection.
...
func newNetAddress(addr net.Addr, services wire.ServiceFlag) (*wire.NetAddress, error) {
	...
	case *socks.ProxiedAddr:
		...
		port = uint16(a.Port)
	default:
		return nil, fmt.Errorf("newNetAddress: unsupported address type %T", addr)
	}
	if ip.IsLoopback() || ip.IsPrivate() || !ip.IsGlobalUnicast() || isCGNAT(ip) {
		ip = nil
		port = 0
	}
