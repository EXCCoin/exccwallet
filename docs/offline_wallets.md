...
```
cd $GOPATH/src/github.com//EXCCoin/exccwallet/cmd/movefunds
go install
```
...
3. Open a terminal and change directory to where unspent.json is. Then, copy 
    config.json from $GOPATH/src/github.com/EXCCoin/exccwallet/cmd/movefunds to 
	this directory.
	```
	cp $GOPATH/src/github.com/EXCCoin//exccwallet/cmd/movefunds/config.json config.json
	```
