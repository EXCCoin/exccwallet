...
import (
	...
	"github.com/EXCCoin/exccwallet/internal/helpers"
...
var handlers = map[string]handler{
	...
	// Reference methods which can't be implemented by exccwallet due to
...
func unimplemented(*Server, interface{}) (interface{}, error) {
	return nil, &exccjson.RPCError{
		Code:    exccjson.ErrRPCUnimplemented,
		Message: "Method unimplemented",
	}
}

// unsupported handles a standard bitcoind RPC request which is
// unsupported by exccwallet due to design differences.
...
// context.
type lazyHandler func() (interface{}, *exccjson.RPCError)
...
func lazyApplyHandler(s *Server, request *exccjson.Request) lazyHandler {
	handlerData, ok := handlers[request.Method]
	if !ok {
		return func() (interface{}, *exccjson.RPCError) {
	...
	return func() (interface{}, *exccjson.RPCError) {
		...
		if err != nil {
			return nil, exccjson.ErrRPCInvalidRequest
		}
...
func makeResponse(id, result interface{}, err error) exccjson.Response {
	idPtr := idPointer(id)
	if err != nil {
		return exccjson.Response{
			ID:    idPtr,
			Error: jsonError(err),
		}
	}
	...
	if err != nil {
		return exccjson.Response{
			ID: idPtr,
			Error: &exccjson.RPCError{
				Code:    exccjson.ErrRPCInternal.Code,
...
func jsonError(err error) *exccjson.RPCError {
	if err == nil {
		return nil
	}
	...
	switch e := err.(type) {
	case exccjson.RPCError:
		return &e
	case *exccjson.RPCError:
		return e
	case DeserializationError:
		code = exccjson.ErrRPCDeserialization
	case InvalidParameterError:
		code = exccjson.ErrRPCInvalidParameter
	case ParseError:
		code = exccjson.ErrRPCParse.Code
	case apperrors.E:
		switch e.ErrorCode {
		case apperrors.ErrWrongPassphrase:
			code = exccjson.ErrRPCWalletPassphraseIncorrect
		}
	}
	return &exccjson.RPCError{
		Code:    code,
		Message: err.Error(),
	}
}
...
func accountAddressIndex(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.AccountAddressIndexCmd)
...
func accountSyncAddressIndex(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.AccountSyncAddressIndexCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string,
	...
	keysesPrecious := make([]*exccutil.AddressSecpPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := exccutil.NewAddressSecpPubKey(
...
func addMultiSigAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.AddMultisigAddressCmd)
	...
	secp256k1Addrs := make([]exccutil.Address, len(cmd.Keys))
	...
	err = chainClient.LoadTxFilter(false, []exccutil.Address{p2shAddr}, nil)
	if err != nil {
		return nil, err
	}
...
func addTicket(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.AddTicketCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func consolidate(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ConsolidateCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	var changeAddr exccutil.Address
	if cmd.Address != nil {
		if *cmd.Address != "" {
...
func createMultiSig(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.CreateMultisigCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	address, err := exccutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
		// above is a valid script, shouldn't happen.
		return nil, err
	}

	return exccjson.CreateMultiSigResult{
...
func dumpPrivKey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.DumpPrivKeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func generateVote(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GenerateVoteCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	resp := &exccjson.GenerateVoteResult{
		Hex: txHex,
	}
...
func getAddressesByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetAddressesByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func getBalance(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetBalanceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	result := exccjson.GetBalanceResult{
	...
	if accountName == "*" {
		...
		var (
			totImmatureCoinbase exccutil.Amount
			totImmatureStakegen exccutil.Amount
			totLocked           exccutil.Amount
			totSpendable        exccutil.Amount
			totUnconfirmed      exccutil.Amount
			totVotingAuthority  exccutil.Amount
			cumTot              exccutil.Amount
		)

		balancesLen := uint32(len(balances))
		result.Balances = make([]exccjson.GetAccountBalanceResult, balancesLen)
		...
		for _, bal := range balances {
			...
			json := exccjson.GetAccountBalanceResult{
...
func getBestBlock(s *Server, icmd interface{}) (interface{}, error) {
	...
	result := &exccjson.GetBestBlockResult{
		Hash:   hash.String(),
		Height: int64(height),
	}
	return result, nil
...
func getInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	// Call down to exccd for all of the information in this command known
	// by them.
...
func decodeAddress(s string, params *chaincfg.Params) (exccutil.Address, error) {
	...
	if len(s) == 66 || len(s) == 130 {
		...
		pubKeyAddr, err := exccutil.NewAddressSecpPubKey(pubKeyBytes,
			params)
		if err != nil {
			return nil, err
		}
	...
	addr, err := exccutil.DecodeAddress(s)
	if err != nil {
		msg := fmt.Sprintf("Invalid address %q: decode failed with %#q", s, err)
		return nil, &exccjson.RPCError{
			Code:    exccjson.ErrRPCInvalidAddressOrKey,
			Message: msg,
		}
	}
...
func getAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
// appear in the blockchain, or any tx that has arrived in the exccd mempool).
// If the most recently-requested address has been used, a new address (the
// next chained address in the keypool) is used.  This can fail if the keypool
// runs out (and will return exccjson.ErrRPCWalletKeypoolRanOut if that happens).
func getAccountAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetAccountAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func getUnconfirmedBalance(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetUnconfirmedBalanceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func importPrivKey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ImportPrivKeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	wif, err := exccutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &exccjson.RPCError{
			Code:    exccjson.ErrRPCInvalidAddressOrKey,
...
func importScript(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ImportScriptCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func createNewAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.CreateNewAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func renameAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.RenameAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func getMultisigOutInfo(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetMultisigOutInfoCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	result := &exccjson.GetMultisigOutInfoResult{
...
func getNewAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetNewAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func getRawChangeAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetRawChangeAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func getReceivedByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetReceivedByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func getReceivedByAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetReceivedByAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func getMasterPubkey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetMasterPubkeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func getStakeInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	resp := &exccjson.GetStakeInfoResult{
...
func getTickets(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetTicketsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	return &exccjson.GetTicketsResult{Hashes: ticketHashStrs}, nil
}

// getTransaction handles a gettransaction request by returning details about
// a single transaction saved by wallet.
func getTransaction(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.GetTransactionCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	if err != nil {
		return nil, &exccjson.RPCError{
			Code:    exccjson.ErrRPCDecodeHexString,
	...
	ret := exccjson.GetTransactionResult{
		TxID:            cmd.Txid,
	...
	var (
		debitTotal  exccutil.Amount
		creditTotal exccutil.Amount // Excludes change
		fee         exccutil.Amount
		negFeeF64   float64
	)
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var outputTotal exccutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += exccutil.Amount(output.Value)
		}
	...
	if len(details.Debits) == 0 {
		...
		ret.Details = make([]exccjson.GetTransactionDetailsResult, 0,
			len(details.Credits))
	} else {
		ret.Details = make([]exccjson.GetTransactionDetailsResult, 1,
			len(details.Credits)+1)

		ret.Details[0] = exccjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Account
			//   Address
	...
	for _, cred := range details.Credits {
		...
		ret.Details = append(ret.Details, exccjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Fee
			Account:  accountName,
...
func getVoteChoices(s *Server, icmd interface{}) (interface{}, error) {
	...
	resp := &exccjson.GetVoteChoicesResult{
		Version: version,
		Choices: make([]exccjson.VoteChoice, len(agendas)),
	}
	...
	for i := range choices {
		resp.Choices[i] = exccjson.VoteChoice{
...
func help(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.HelpCmd)
	// The "help" RPC must use an HTTP POST client when calling down to exccd
...
func listAccounts(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ListAccountsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func listReceivedByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ListReceivedByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	jsonResults := make([]exccjson.ListReceivedByAccountResult, 0, len(results))
	for _, result := range results {
		jsonResults = append(jsonResults, exccjson.ListReceivedByAccountResult{
...
func listReceivedByAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ListReceivedByAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	ret := make([]exccjson.ListReceivedByAddressResult, numAddresses)
	idx := 0
	for address, addrData := range allAddrData {
		ret[idx] = exccjson.ListReceivedByAddressResult{
			Address:       address,
...
func listSinceBlock(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ListSinceBlockCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	res := exccjson.ListSinceBlockResult{
...
func listScripts(s *Server, icmd interface{}) (interface{}, error) {
	...
	listScriptsResultSIs := make([]exccjson.ScriptInfo, len(redeemScripts))
	for i, redeemScript := range redeemScripts {
		p2shAddr, err := exccutil.NewAddressScriptHash(redeemScript,
			w.ChainParams())
		if err != nil {
			return nil, err
		}
		listScriptsResultSIs[i] = exccjson.ScriptInfo{
			Hash160:      hex.EncodeToString(p2shAddr.Hash160()[:]),
			Address:      p2shAddr.EncodeAddress(),
			RedeemScript: hex.EncodeToString(redeemScript),
		}
	}
	return &exccjson.ListScriptsResult{Scripts: listScriptsResultSIs}, nil
}

// listTransactions handles a listtransactions request by returning an
// array of maps with details of sent and recevied wallet transactions.
func listTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ListTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func listAddressTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ListAddressTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func listAllTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ListAllTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func listUnspent(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ListUnspentCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func lockUnspent(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.LockUnspentCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func purchaseTicket(s *Server, icmd interface{}) (interface{}, error) {
	...
	cmd := icmd.(*exccjson.PurchaseTicketCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}

	spendLimit, err := exccutil.NewAmount(cmd.SpendLimit)
	if err != nil {
		return nil, err
	}
	if spendLimit < 0 {
	...
	var ticketAddr exccutil.Address
	...
	if cmd.TicketFee != nil {
		ticketFee, err = exccutil.NewAmount(*cmd.TicketFee)
		if err != nil {
			return nil, err
		}
	}
...
// and amounts.
func makeOutputs(pairs map[string]exccutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
...
// All errors are returned in exccjson.RPCError format
func sendPairs(w *wallet.Wallet, amounts map[string]exccutil.Amount,
...
func redeemMultiSigOut(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.RedeemMultiSigOutCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	srtc := &exccjson.SignRawTransactionCmd{
		RawTx:    txDataStr,
		Inputs:   &rtis,
		PrivKeys: &[]string{},
		Flags:    &sigHashAll,
	...
	srtTyped := signedTxResult.(exccjson.SignRawTransactionResult)
	return exccjson.RedeemMultiSigOutResult(srtTyped), nil
}
...
func redeemMultiSigOuts(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.RedeemMultiSigOutsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	p2shAddr, ok := addr.(*exccutil.AddressScriptHash)
	if !ok {
	...
	itr := uint32(0)
	rmsoResults := make([]exccjson.RedeemMultiSigOutResult, len(msos))
	for i, mso := range msos {
		if itr > max {
			break
		}

		rmsoRequest := &exccjson.RedeemMultiSigOutCmd{
		...
		redeemResultTyped := redeemResult.(exccjson.RedeemMultiSigOutResult)
		rmsoResults[i] = redeemResultTyped

		itr++
	}

	return exccjson.RedeemMultiSigOutsResult{Results: rmsoResults}, nil
}
...
func rescanWallet(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.RescanWalletCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func stakePoolUserInfo(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.StakePoolUserInfoCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}

	userAddr, err := exccutil.DecodeAddress(cmd.User)
	if err != nil {
		return nil, err
	}
	...
	resp := new(exccjson.StakePoolUserInfoResult)
	for _, ticket := range spui.Tickets {
		var ticketRes exccjson.PoolUserTicket
...
func ticketsForAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.TicketsForAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}

	addr, err := exccutil.DecodeAddress(cmd.Address)
	if err != nil {
		return nil, err
	}
	...
	return exccjson.TicketsForAddressResult{Tickets: ticketHashStrs}, nil
}
...
func sendFrom(s *Server, icmd interface{}) (interface{}, error) {
	...
	amt, err := exccutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]exccutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(s *Server, icmd interface{}) (interface{}, error) {
	...
	// Recreate address/amount pairs, using exccutil.Amount.
	pairs := make(map[string]exccutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := exccutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func sendToAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.SendToAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func sendToMultiSig(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.SendToMultiSigCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	account := uint32(udb.DefaultAccountNum)
	amount, err := exccutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	nrequired := int8(*cmd.NRequired)
	minconf := int32(*cmd.MinConf)
	pubkeys := make([]*exccutil.AddressSecpPubKey, len(cmd.Pubkeys))
	...
	result := &exccjson.SendToMultiSigResult{
	...
	err = chainClient.LoadTxFilter(false, []exccutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func setTicketFee(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.SetTicketFeeCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	incr, err := exccutil.NewAmount(cmd.Fee)
	if err != nil {
		return nil, err
	}
...
func setTxFee(s *Server, icmd interface{}) (interface{}, error) {
	...
	relayFee, err := exccutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
...
func setVoteChoice(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.SetVoteChoiceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func signMessage(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.SignMessageCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func signRawTransaction(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.SignRawTransactionCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	// TODO: really we probably should look these up with exccd anyway to
	// make sure that they match the blockchain if present.
	inputs := make(map[wire.OutPoint][]byte)
	scripts := make(map[string][]byte)
	var cmdInputs []exccjson.RawTxInput
	if cmd.Inputs != nil {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := exccutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	var keys map[string]*exccutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*exccutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := exccutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			case chainec.ECTypeSecp256k1:
				addr, err = exccutil.NewAddressSecpPubKey(wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, DeserializationError{err}
				}
			case chainec.ECTypeEdwards:
				addr, err = exccutil.NewAddressEdwardsPubKey(
					wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, DeserializationError{err}
			...
			case chainec.ECTypeSecSchnorr:
				addr, err = exccutil.NewAddressSecSchnorrPubKey(
					wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, DeserializationError{err}
	...
	signErrors := make([]exccjson.SignRawTransactionError, 0, len(signErrs))
	for _, e := range signErrs {
		input := tx.TxIn[e.InputIndex]
		signErrors = append(signErrors, exccjson.SignRawTransactionError{
	...
	return exccjson.SignRawTransactionResult{
...
func signRawTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.SignRawTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	results := make([]exccjson.SignRawTransactionResult, len(cmd.RawTxs))
...
func startAutoBuyer(s *Server, icmd interface{}) (interface{}, error) {
	...
	if cmd.VotingAddress != nil {
		var votingAddress exccutil.Address
	...
	var poolAddress exccutil.Address
	if cmd.PoolAddress != nil {
		if *cmd.PoolAddress != "" {
...
func sweepAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.SweepAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
	...
	if cmd.FeePerKb != nil {
		var err error
		feePerKb, err = exccutil.NewAmount(*cmd.FeePerKb)
		if err != nil {
			return nil, err
		}
	}
	...
	res := &exccjson.SweepAccountResult{
...
func validateAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.ValidateAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}

	result := exccjson.ValidateAddressWalletResult{}
	addr, err := decodeAddress(cmd.Address, w.ChainParams())
	if err != nil {
...
func verifyMessage(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.VerifyMessageCmd)
...
func version(s *Server, icmd interface{}) (interface{}, error) {
	var resp map[string]exccjson.VersionResult
	chainClient, ok := s.requireChainClient()
	if ok {
		var err error
		...
		resp = make(map[string]exccjson.VersionResult)
	}

	resp["exccwalletjsonrpcapi"] = exccjson.VersionResult{
...
func walletInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	return &exccjson.WalletInfoResult{
...
func walletPassphrase(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.WalletPassphraseCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
...
func walletPassphraseChange(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.WalletPassphraseChangeCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, &ErrUnloadedWallet
	}
